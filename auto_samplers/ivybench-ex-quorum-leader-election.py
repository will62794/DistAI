...
def add_checked_candidates(candidate0, candidate1):
	with open('../configs/ivybench-ex-quorum-leader-election.txt', 'a') as config_file:
		if candidate0:
...
def sample(max_iter=50):
	...
	while stopping_criteria is False:
		...
		for retry in range(10):
			majority = rng.integers(0, 2, size=(nset_num), dtype=bool)
			member = rng.integers(0, 2, size=(node_num, nset_num), dtype=bool)
			if (forall_func_3(nset_num, nset_num, nset_num, (lambda T,Y,S : (not ((majority[S]) and (majority[Y]))) or (exists_func_1(node_num, (lambda N : (member[N, S]) and (member[N, T]))))))):
...
if __name__ == '__main__':
	...
	end_time = time.time()
	df.to_csv('../traces/ivybench-ex-quorum-leader-election.csv', index=False)
	print('Simulation finished. Trace written to traces/ivybench-ex-quorum-leader-election.csv')
