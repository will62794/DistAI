...
def route_prec(p, sw0, sw1, sw2):
	...
	if not (route_dom[dst[p], sw1]):
		return False
	if not ((route_tc[dst[p], sw1, sw2]) and ((sw1) != (sw2)) and (forall_func_1(node_num, (lambda Z : (not ((route_tc[dst[p], sw1, Z]) and ((sw1) != (Z)))) or (route_tc[dst[p], sw2, Z]))))):
		return False
	return True
...
if __name__ == '__main__':
	...
	end_time = time.time()
	df.to_csv('../traces/ivybench-i4-learning-switch.csv', index=False)
	print('Simulation finished. Trace written to traces/ivybench-i4-learning-switch.csv')
